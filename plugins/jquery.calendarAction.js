...
Date.prototype.DateCalc = function(pattern){
	...
	if (pattern.match(/^-?[0-9]+ year$/g)!=null)
	{
		...
		return new Date(this.setYear(this.getFullYear()+parseInt(pattern.match(/^-?[0-9]+/g))));
	}
	//月加算
	if (pattern.match(/^-?[0-9]+ month$/g)!=null)
	{
		month+=parseInt(pattern.match(/^-?[0-9]+/g));
		return new Date(this.setMonth(this.getMonth()+parseInt(pattern.match(/^-?[0-9]+/g))));
		//年末チェック
	...
	if (pattern.match(/^-?[0-9]+ day$/g)!=null)
	{
		...
		return new Date(this.setDate(this.getDate()+parseInt(pattern.match(/^-?[0-9]+/g))));
	}
	//年初日
