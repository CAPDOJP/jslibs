...
jQuery.fn.imgSlider = function(options){
	...
	return $(this).each(function(){
		...
		.on('mousedown',function(e){
			...
			scrollbar.css({
				'bottom':'auto',
				'left':(scrollPos.left-window.pageXOffset).toString()+'px',
				'top':scrollPos.top.toString()+'px',
				'position':'fixed'
			});
			/* コンテナスクロール位置保持 */
			targetPos=target.scrollLeft();
			/* ドラッグ開始位置保持 */
		...
		$(window).on({
			'mousemove':function(e){
				...
				/* コンテナ移動 */
				target.scrollLeft(targetPos+(e.clientX-mousedownPos)/ratio);
				e.preventDefault();
				e.stopPropagation();
			},
			'mouseup':function(e){
				...
				scrollbar.css({
					'bottom':'5px',
					'left':(scrollPos.left-window.pageXOffset+target.scrollLeft()).toString()+'px',
					'top':'auto',
					'position':'absolute'
				});
				/* キャプチャ終了 */
				capture=false;
				/* スクロールバー非表示 */
				if (!target.positionContains(e.clientX,e.clientX)) scrollbar.fadeOut('slow');
				e.preventDefault();
				e.stopPropagation();
			}
		});
