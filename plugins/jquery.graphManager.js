	...
	redraw:function(){
		...
		$.each(this.values,function(index){
			...
			$.each(values,function(index){
				if (my.scale.max==null && my.maxvalue<Math.ceil(values[index])) my.maxvalue=Math.ceil(values[index]);
				if (my.scale.min==null && my.minvalue>Math.floor(values[index])) my.minvalue=Math.floor(values[index]);
			});
		});
