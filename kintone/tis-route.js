		...
		var addmarker=function(latlng,childindex,colorsindex,label){
			...
			if (options.markerfontsize!=null)
			{
				marker=new google.maps.Marker({
					map:map,
					position:latlng,
					icon:'https://chart.googleapis.com/chart?chst=d_map_spin&chld=0.75|0|'+colors[colorsindex].back+'|'+options.markerfontsize.toString()+'|_|'+((childindex==-1)?'':childindex.toString())
				});
			}
			else
			{
