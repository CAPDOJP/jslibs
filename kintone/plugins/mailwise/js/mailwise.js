...
(function($,PLUGIN_ID,spinner){
	...
	kintone.events.on(events.lists,function(event){
		...
		kintone.api(kintone.api.url('/k/v1/app/form/fields',true),'GET',{app:kintone.app.getId()},function(resp){
			...
			headspace.append(mailform.css({'display':'inline-block'}));
	...
	kintone.events.on(events.show,function(event){
		...
		kintone.api(kintone.api.url('/k/v1/app/form/fields',true),'GET',{app:kintone.app.getId()},function(resp){
			vars.fields=resp.properties;
			/* create form */
			var headspace=$('.gaia-argoui-app-edit-buttons');
			var mailform=$('<form>').attr('action','/m/mw.cgi').attr('method','POST').attr('target','_blank');
			...
			if(vars.config.historyspace)
			{
				...
				vars.history.prev=$('<a href="javascript:void(0)">').addClass('pager-prev-gaia').addClass('mailwise-feed').text('前へ').on('click',function(){
					/* reset history mails */
					vars.history.offset-=vars.history.limit;
					functions.resethistories();
				});
