		...
		build:function(filter,cell){
			if (filter.length!=0)
			{
				for (var i=0;i<filter.length;i++)
				{
					...
					var color={
						course:vars.courses[filter[i]['coursecode'].value].color,
					...
					for (var i2=1;i2<vars.tooltips.length;i2++) inner+='<span>'+filter[i][vars.tooltips[i2]].value+'</span>';
					inner+='</p>';
		...
		load:function(){
			...
			functions.loaddatas(kintone.app.getId(),function(){
				...
				vars.table.contents.find('td').each(function(index){
					...
					Array.prototype.push.apply(filter,$.createschedule(vars.apps[vars.config['student']],vars.apps[vars.coursekeys[0]],filter,day.getDay()));
					/* sort */
