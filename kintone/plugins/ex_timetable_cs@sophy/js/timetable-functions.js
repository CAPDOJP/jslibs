...
jQuery.extend({
	loadapps:function(counter,param,splash,callback){
		kintone.api(kintone.api.url('/k/v1/app/form/fields',true),'GET',{app:param[counter].app},function(resp){
			...
			var body={
				...
				query:'order by $id asc limit '+param[counter].limit.toString()+' offset '+param[counter].offset.toString()
			};
	...
	createschedule:function(studentrecords,courserecords,checkrecords,weekday){
		var res=[];
		for (var i=0;i<studentrecords.length;i++)
		{
			var student=studentrecords[i];
			var course=$.grep(courserecords,function(item,index){return (item['code'].value==student['coursecode'].value);})[0];
			var coursegrade=$.coursegrade(course,student['gradecode'].value);
			for (var i2=0;i2<student['coursetable'].value.length;i2++)
			{
				var row=student['coursetable'].value[i2];
				if (vars.week.indexOf(row['courseweek'].value)==weekday)
				{
					var reserved=$.grep(checkrecords,function(item,index){
						var exists=0;
						if (item['studentcode'].value==student['$id'].value) exists++;
						if (item['appcode'].value==vars.coursekeys[0]) exists++;
						return exists==2;
					});
					if (reserved.length==0)
						res.push({
							studentcode:student['$id'].value,
							studentname:student['name'].value,
							appcode:vars.coursekeys[0],
							appname:vars.courses[vars.coursekeys[0]].name,
							coursecode:course['code'].value,
							coursename:course['name'].value,
							date:day.format('Y-m-d'),
							starttime:row['coursestarttime'].value,
							hours:coursegrade['hours'].value,
							baserecordid:null,
							transfered:0,
							transferpending:0,
							transferlimit:null
