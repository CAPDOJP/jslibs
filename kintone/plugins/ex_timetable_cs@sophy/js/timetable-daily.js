...
(function($,PLUGIN_ID){
	...
	var functions={
		...
		build:function(filter){
			...
			vars.table.insertrow(null,function(row){
				...
				if (filter.length!=0)
				{
					...
					{
						/* create cell */
						var color=vars.courses[filter[i]['coursecode'].value].color;
		...
		mergeaftervalue:function(row,from,to,filter,color){
			...
			for (var i=1;i<vars.tooltips.length;i++) inner+='<span>'+filter[vars.tooltips[i]].value+'</span>';
			inner+='</p>';
			$('body').append(
				balloon.css({
		...
		load:function(){
			...
			functions.loaddatas(kintone.app.getId(),function(){
				...
				Array.prototype.push.apply(records,$.createschedule(vars.apps[vars.config['student']],vars.apps[vars.coursekeys[0]],records,vars.date.getDay()));
				/* sort */
