...
jQuery.extend({
	...
	createschedule:function(studentrecords,courserecords,checkrecords,coursecode,coursename,week,day){
		var res=[];
			...
			for (var i2=0;i2<student['coursetable'].value.length;i2++)
			{
				...
				if (week.indexOf(row['courseweek'].value)==day.getDay())
				{
					var reserved=$.grep(checkrecords,function(item,index){
						...
						if (item['appcode'].value==coursecode) exists++;
						return exists==2;
					});
					if (reserved.length==0)
						res.push({
							...
							appcode:{value:coursecode},
							appname:{value:coursename},
