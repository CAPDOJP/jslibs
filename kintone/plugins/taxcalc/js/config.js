...
(function($,PLUGIN_ID){
		...
		kintone.api(kintone.api.url('/k/v1/app/form/fields',true),'GET',{app:kintone.app.getId()},function(resp){
			...
			$.each(sorted,function(index){
				if (sorted[index] in vars.fieldinfos)
				{
					...
					{
						case 'CALC':
							if (fieldinfos.format.match(/NUMBER/g))
								if (fieldinfo.tablecode.length!=0)
									$('select#price').append($('<option>').attr('value',values.code).text(values.label));
							break;
						case 'DATE':
							$('select#taxdate').append($('<option>').attr('value',values.code).text(values.label));
							break;
						case 'NUMBER':
							$('select#subtotal').append($('<option>').attr('value',values.code).text(values.label));
							$('select#tax').append($('<option>').attr('value',values.code).text(values.label));
							$('select#price').append($('<option>').attr('value',values.code).text(values.label));
							break;
						case 'DROP_DOWN':
						case 'RADIO_BUTTON':
							if (fieldinfo.tablecode.length!=0)
								$('select#taxsegment').append($('<option>').attr('value',values.code).text(values.label));
							break;
					}
				}
			});
			...
			if (Object.keys(config).length!==0)
			{
				...
				$.each(relations,function(index){
					...
					$('select#price',row).val(relations[index].price);
	...
	$('button#submit').on('click',function(e){
		...
		for (var i=0;i<vars.relationtable.rows.length;i++)
		{
			...
			if ($('select#price',row).val().length==0) continue;
			if ($('select#taxsegment',row).val().length==0) continue;
			if ($('select#totax',row).val().length==0) continue;
			if (vars.fieldinfos[$('select#price',row).val()].tablecode!=vars.fieldinfos[$('select#taxsegment',row).val()].tablecode)
			{
				swal('Error!','金額フィールドと課税区分フィールドの指定は同一テーブルにして下さい。','error');
				return;
			}
			relations.push({
				subtotal:$('select#subtotal',row).val(),
				tax:$('select#tax',row).val(),
				price:$('select#price',row).val(),
				taxsegment:$('select#taxsegment',row).val(),
				totax:$('select#totax',row).val(),
				tablecode:vars.fieldinfos[$('select#price',row).val()].tablecode
			});
		}
		/* setup config */
