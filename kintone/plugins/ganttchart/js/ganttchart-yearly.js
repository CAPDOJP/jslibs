	...
	var functions={
			...
			vars.table.insertrow(null,function(row){
				...
				if (filter.length!=0)
				{
					for (var i=0;i<filter.length;i++)
					{
						...
						var toindex=0;
						if (from<0) from=0;
						if (to>vars.datecalc.diffmonths) to=vars.datecalc.diffmonths;
		...
		load:function(){
			...
			functions.loaddatas(kintone.app.getId(),function(){
				...
				{
					if (i!=0 && columns.cache.format('Y')!=columns.cache.calc('-1 month').format('Y'))
					{
					...
					head.eq(1).append($('<th>').append(spacer.clone(false).text(columns.cache.getMonth()+1)));
					template.append($('<td>'));
				...
				vars.table=$('<table id="ganttchart" class="customview-table ganttchart '+((vars.config['scalefixed']=='1')?'cellfixed':'')+'">').mergetable({
					...
					mergetrigger:function(caller,cell,rowindex,cellfrom,cellto){
						...
						query+='&'+vars.config['todate']+'='+vars.fromdate.calc(tomonth.toString()+' month').calc('1 month').calc('-1 day').format('Y-m-d');
