...
(function($,PLUGIN_ID){
	...
	var functions={
		...
		mergeaftervalue:function(row,from,to,filter,color){
			...
			if (vars.config['registeredonly']=='1') row.find('td').eq(from).on('click',function(){
				sessionStorage.setItem('fromdate_ganttchart-monthly',vars.fromdate.format('Y-m-d').dateformat());
				sessionStorage.setItem('todate_ganttchart-monthly',vars.todate.format('Y-m-d').dateformat());
	...
	kintone.events.on(events.lists,function(event){
		...
		/* setup date value */
		if (sessionStorage.getItem('fromdate_ganttchart-monthly'))
		{
			vars.fromdate=new Date(sessionStorage.getItem('fromdate_ganttchart-monthly'));
			sessionStorage.removeItem('fromdate_ganttchart-monthly');
		}
		if (sessionStorage.getItem('todate_ganttchart-monthly'))
		{
			vars.todate=new Date(sessionStorage.getItem('todate_ganttchart-monthly'));
			sessionStorage.removeItem('todate_ganttchart-monthly');
		}
