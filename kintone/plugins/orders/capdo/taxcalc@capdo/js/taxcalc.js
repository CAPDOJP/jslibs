					...
					if ('relation' in vars.config)
					{
						...
						kintone.events.on(vars.events,function(event){
							...
							for (var i=0;i<vars.relations.length;i++)
							{
								...
								event.record[relation.subtotal].value=(calc.able-calc.tax).toString();
								event.record[relation.subtotal_free].value=calc.free.toString();
