...
(function($,PLUGIN_ID){
		...
		kintone.api(kintone.api.url('/k/v1/app/form/fields',true),'GET',{app:kintone.app.getId()},function(resp){
				...
				if (sorted[index] in vars.fieldinfos)
				{
					...
					switch (fieldinfo.type)
					{
						...
						case 'FILE':
							...
							if (!fieldinfo.tablecode) $('select#revisionattachment').append($('<option>').attr('value',fieldinfo.code).text(fieldinfo.label));
							break;
						case 'MULTI_LINE_TEXT':
			...
			functions.loadapps(function(){
				...
				if (Object.keys(config).length!==0)
				{
					...
					$('select#revisionattachment').val(config['revisionattachment']);
	...
	$('button#submit').on('click',function(e){
		...
		{
			swal('Error!','注文担当者フィールドを選択して下さい。','error');
			return;
		}
		...
		if ($('select#revisionattachment').val()=='')
		{
			swal('Error!','訂正添付ファイルフィールドを選択して下さい。','error');
			return;
		}
		...
		config['revisionattachment']=$('select#revisionattachment').val();
